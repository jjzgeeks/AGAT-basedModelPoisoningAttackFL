...
def GAT_to_GAE(parameters,  FL_adj):
    ...
    """cifar10 dataset"""
    mali_local_model = np.sum(new_features, axis=0) / new_features.shape[0] * np.random.normal(0.0, 1.0, new_features.shape[1]) # new_features.shape[0] is column 2e-2
    
    """svhn dataset"""
    #mali_local_model = np.sum(new_features, axis=0) / new_features.shape[0] * np.random.normal(0.0, 1.0, new_features.shape[1])*3e-5 # new_features.shape[0] is column
