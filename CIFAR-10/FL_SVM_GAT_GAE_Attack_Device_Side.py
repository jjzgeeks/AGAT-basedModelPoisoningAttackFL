...
from utilities.data_processing import load_mnist_return_required_digits, load_svhn_return_required_digits, get_clients, get_total_from_clients, load_cifar10_return_required_digits, create_kmeans_clusters, load_fashion_mnist_return_required_digits
...
class Federated_SVM:
    ...
    def average_aggregator(self, parameter_list, num_attacks):
    ...
    def fit(self, g_iters, aggregator, num_malicious):
        ...
        for i in range(0, g_iters):
            ...
            num_change = math.ceil(self.n_clients / 1)
            ...
            #p1 = [self.clients[k].w * np.random.normal(0, 1, len(w_best)) for k in
            #      range(0, num_change)]  # all weights of clients
            p1 = [self.clients[k].w  for k in range(0, num_change)]  # all weights of clients
            ...
            parameter_list = copy.deepcopy(p1)
            ...
            if i < 1:
                p_list = np.array([self.clients[k].w * np.random.normal(0, 1, len(w_best))  for k in range(0, self.n_clients)])
                ...
                for mali in range(num_malicious):
                    ...
                    #initialization_graph_matrix = np.random.random((N, N))
                    ...
                    #print(attack_node)
                    #attack_base = attack_base * np.random.normal(0, 1, len(w_best))
                ...
                w_agg = aggregator(w_all, num_malicious)
...
if __name__ == '__main__':
    ...
    for x in dataset:
        ...
        if x == "mnist":
            ...
            data = load_svhn_return_required_digits(1, 9)
        else:
        ...
        for n_clients in num_clients_index:
            ...
            num_global_commu_round_index = [110, 100, 200, 300]
            ...
            for m_clients in num_malicious_clients_index:
                ...
                savemat("./FL_GAT_GAE_results_{}_{}_{}_{}_{}.mat".format(x, n_clients, n_global_commu_round, n_iters, m_clients),
                        {"Global_model_loss": Loss, "Global_model_accuracy": global_accuracy,
                         "Local_model_accuracy": local_clients_accuracy, "KL_divergence": KL_div, "cosine_distance": cosine_distance, "length_jains_set": length_jains_set, "angle_jains_set": angle_jains_set})
                ...
                for i in range(n_clients):
                    ...
                    plt.plot(range(n_global_commu_round), local_clients_accuracy[i][1:n_global_commu_round + 1], label=f'Device {i}')
                plt.legend()
                plt.xlabel('Communication rounds')
