...
# def load_svhn_return_required_digits(m, n):
...
#         image, label = batch
#         x_train.append(np.vstack(image.tolist()))
#         y_train.append(label.tolist())
#     x_train = np.array(x_train)
#     #print(x_train.shape) x_train shape is: (73257, 3, 32, 32) y_train (73257,)
#
#     for b in test_loader:
#         img, lab = b
#         x_test.append(np.vstack(img.tolist()))
#         y_test.append(lab.tolist())
#     x_test = np.array(x_test)
#     x_train_gray = rgb2gray(x_train)
#     x_test_gray = rgb2gray(x_test)
#     #print(x_train_gray.shape) (73257, 3, 32)
#
#     x_total = np.concatenate((x_train_gray, x_test_gray), axis=0)
#     y_total = np.concatenate((y_train, y_test), axis=0)
#     y_total = y_total.flatten()  # Flatten a 2d numpy array into 1d array. y_total shape is  [6 5 6 ... 7 6 7]
#     # print(y_total.shape) (99289,)
#
#
#     # reshaping the data
#     x_total = x_total.astype('float32') / 255
#     x_total = x_total.reshape(x_total.shape[0], 32 * 3)  # x_total shape is (99289, 96)
#
#
#     x_n1 = x_total[y_total == m]
#     y_n1 = y_total[y_total == m]
...
def load_svhn_return_required_digits(m, n):
  ...
  svhn_dataset, info = tfds.load('svhn_cropped', split=['train', 'test'], with_info=True)

  # Extract training and test sets
  svhn_train = svhn_dataset[0]
  svhn_test = svhn_dataset[1]
...
def load_imdb_return_required_digits(m, n):
  ...
  y_total = np.concatenate((y_train, y_test), axis=0)
  y_total = y_total.flatten()  # Flatten a 2d numpy array into 1d array. y_total shape is  (60000,)
  x_total = x_total.reshape(x_total.shape[0], max_length)
  x_n1 = x_total[y_total == m]
  y_n1 = y_total[y_total == m]
  x_n2 = x_total[y_total == n]
  y_n2 = y_total[y_total == n]
