...
from .gae_utils import eval_gae, make_sparse
from .models import GAE
from .preprocessing import mask_test_edges, preprocess_graph



def GAT_to_GAE(parameters,  FL_adj):
    ...
    """mnist dataset"""
    #mali_local_model = np.sum(new_features, axis=0) / new_features.shape[0] * np.random.uniform(-1.0, 1.0, new_features.shape[1]) # new_features.shape[0] is column
    #mali_local_model = np.sum(new_features, axis=0) / new_features.shape[0] * np.random.uniform(0.0, 1.0, new_features.shape[1])*1e-2 # new_features.shape[0] is column

    """cifar10 dataset"""
    #mali_local_model = np.sum(new_features, axis=0) / new_features.shape[0] * np.random.normal(0.0, 1.0, new_features.shape[1])*2e-2 # new_features.shape[0] is column
    
    """svhn dataset"""
    mali_local_model = np.sum(new_features, axis=0) / new_features.shape[0] * np.random.normal(0.0, 1.0, new_features.shape[1])*5e-3 # new_features.shape[0] is column
