...
class Federated_SVM:
    def __init__(self, x_train ,y_train, n_clients, n_iters, val=True, val_type='k_fold', k=5, opt='mini_batch_GD',
                 batch_size=64, learning_rate=0.005, lambda_param=0.01):
    ...
    def fit(self, g_iters, aggregator, num_attcks, noise):
        ...
        for i in range(0, g_iters):
            ...
            for k in range(0, self.n_clients):
                ...
                b_list.append(self.clients[k].b)
            b_list = np.array(b_list)
            bias = np.mean(b_list, axis=0)
    ...
    for x in dataset:
      ...
      if x == "mnist":
         ...
         #noise = noise_mat['fmnist_noise']
          ...
          noise_mat = sio.loadmat('./utilities/svhn_noise.mat')
      ...
      for n_clients in num_clients_index:
         ...
         for num_attcks in num_attacks_index:
           Loss, global_accuracy, local_clients_accuracy, KL_div, cosine_distance, length_jains_set, angle_jains_set = f_svm.fit(n_global_commu_round, f_svm.average_aggregator, num_attcks, noise)
